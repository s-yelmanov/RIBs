...
public class Executor {
    private static var сompositeCancellable = CompositeCancellable()
    ...
    public static func execute(withDelay delay: TimeInterval, maxFrameDuration: Int = 33, logic: @escaping () -> ()) {
        ...
        cancellable = Timer.publish(every: period, on: .main, in: .common)
            ...
            .sink(
                receiveCompletion: { [weak cancellable] _ in
                    guard let cancellable = cancellable else { return }
                    сompositeCancellable.remove(cancellable)
                },
                receiveValue: { _ in
        ...
        cancellable?.store(in: сompositeCancellable)
    }
}
